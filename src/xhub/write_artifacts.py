...
def handler(event, context):
    ...
    try:
        ...
        # Iterate through each artifact and store it in DynamoDB
        for artifact in artifacts:
            ...
            if not artifact_name or not artifact_files:
                return {
                    "statusCode": 400,
                    "body": "Missing required parameters in the artifact.",
                }

            # Generate a unique identifier for the artifact
            artifact_id = str(uuid.uuid4())
            artifact_ids_by_name[artifact_name] = artifact_id

            # Store the artifact in DynamoDB
            dynamodb.put_item(
        ...
        print(f"Error storing artifacts in DynamoDB: {e}")
